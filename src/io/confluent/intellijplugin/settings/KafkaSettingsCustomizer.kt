...
class KafkaSettingsCustomizer(
    ...
    override val url = StringNamedField(
        ConnectionData::uri,
        ModificationKey(KafkaMessagesBundle.message("settings.url")),
        connectionData
    )
        .apply {
    ...
    internal val registryType = RadioGroupField(
        KafkaConnectionData::registryType,
        ModificationKey(KafkaMessagesBundle.message("schema.registry.type.label")), connectionData,
        KafkaRegistryType.entries
    )

    private val brokerSettings =
        KafkaBrokerSettings(project, connectionData, uiDisposable, coroutineScope, url, registryType)
    private val registrySettings =
        KafkaRegistrySettings(project, connectionData, uiDisposable, coroutineScope, registryType)
    ...
    override fun getDefaultComponent(
        fields: List<WrappedComponent<in KafkaConnectionData>>,
        conn: KafkaConnectionData
    ) = panel {
