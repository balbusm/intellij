...
import io.confluent.intellijplugin.aws.ui.external.StaticAwsSettingsInfo
import io.confluent.intellijplugin.aws.utils.AwsSettingsConst
...
import io.confluent.intellijplugin.rfs.SchemaRegistryAuthType
...
class KafkaRegistrySettings(
    ...
    private val confluentPropertiesCredentialsHolder = CredentialsHolder(
        connectionData, KafkaConnectionData.CONFIG_REGISTRY_KEY,
    ...
    internal val confluentSource = RadioGroupField(
        KafkaConnectionData::registryConfSource,
    ...
    internal val confluentSchemaAuth = RadioComboBox(
        SchemaRegistryAuthType.entries.toTypedArray(),
        SchemaRegistryAuthType.NOT_SPECIFIED
    ).apply {
        addItemListener {
    ...
    private val awsCredentials =
        CredentialsHolder(connectionData, AwsCompatibleConnectionData.SECRET_KEY_ID, uiDisposable, coroutineScope)
    ...
    private val useBrokerSslCheckbox = CheckBoxField(
        KafkaConnectionData::registryUseBrokerSsl, USE_BROKER_SSL,
        connectionData
    )
    ...
    private fun setRegistryProperties(properties: Map<String, String>) {
        ...
        val isBasicAuth =
            properties[SchemaRegistryClientConfig.BASIC_AUTH_CREDENTIALS_SOURCE] == SUPPORT_REGISTRY_BASIC_AUTH_TYPE
        val isBearerAuth =
            properties[SchemaRegistryClientConfig.BEARER_AUTH_CREDENTIALS_SOURCE] == SUPPORT_REGISTRY_BEARER_AUTH_TYPE
        when {
            isBasicAuth -> {
            ...
            isBearerAuth -> {
                ...
                confluentBearerToken.component.text =
                    properties[SchemaRegistryClientConfig.BEARER_AUTH_TOKEN_CONFIG] ?: ""
            }
        ...
        val keystoreLocation =
            properties[SchemaRegistryClientConfig.CLIENT_NAMESPACE + SslConfigs.SSL_KEYSTORE_LOCATION_CONFIG] ?: ""
        confluentSslComponent.applyConfig(
            KafkaSslConfig(
    ...
    private fun getRegistryProperties(): Map<String, String?> {
        ...
        val ssl = if (!useBrokerSslCheckbox.checkBoxField.isSelected) {
            ...
            result += mapOf(
                KafkaSslUtils.SSL_CA_LOCATION to config.caCertificate.ifBlank { null },
            ...
            mapOf(
                SchemaRegistryClientConfig.CLIENT_NAMESPACE + SslConfigs.SSL_TRUSTSTORE_LOCATION_CONFIG to null,
    ...
    private fun initGlueSettings(settings: AwsSettingsComponentForKafka) {
        ...
        val info = jsonSettings?.let { BdtJson.fromJsonToClass(it, StaticAwsSettingsInfo::class.java) }
            ?: StaticAwsSettingsInfo(
    ...
    companion object {
        ...
        private val USE_BROKER_SSL =
            ModificationKey(KafkaMessagesBundle.message("kafka.registry.use.broker.ssl.settings.checkbox"))
    }
}
