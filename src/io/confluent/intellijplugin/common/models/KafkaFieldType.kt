...
enum class KafkaFieldType(@Nls val title: String) {
    ...
    fun getDeserializationClass(dataManager: KafkaDataManager, consumerField: ConsumerProducerFieldConfig) =
        when (this) {
    ...
    fun getSerializer(dataManager: KafkaDataManager, producerField: ConsumerProducerFieldConfig) = when (this) {
        ...
        SCHEMA_REGISTRY -> {
            ...
            when (dataManager.registryType) {
                KafkaRegistryType.NONE -> error("Non exists")
                KafkaRegistryType.CONFLUENT -> when (registryFormat) {
                    KafkaRegistryFormat.AVRO -> BdtKafkaAvroSerializer(
                        dataManager.client.confluentRegistryClient?.internalClient,
                        producerField.schemaName, producerField.parsedSchema
                    )

                    KafkaRegistryFormat.PROTOBUF -> BdtKafkaProtobufSerializer(
                        dataManager.client.confluentRegistryClient?.internalClient,
                        producerField.schemaName, producerField.parsedSchema
                    )

                    KafkaRegistryFormat.JSON -> BdtKafkaJsonSchemaSerializer(
                        dataManager.client.confluentRegistryClient?.internalClient,
