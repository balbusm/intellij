...
import io.confluent.intellijplugin.aws.ui.external.AwsSettingsComponentForKafka
import io.confluent.intellijplugin.core.connection.exception.BdtConnectionException
import io.confluent.intellijplugin.core.connection.exception.BdtHostUnavailableException
import io.confluent.intellijplugin.core.connection.exception.BdtUnexpectedConnectionException
import io.confluent.intellijplugin.core.connection.tunnel.BdtSshTunnelService.createIfRequired
...
import io.confluent.intellijplugin.rfs.KafkaPropertySource
...
class KafkaClient(
        ...
        when (connectionData.registryType) {
            ...
            KafkaRegistryType.CONFLUENT -> {
                confluentRegistryClient =
                    ConfluentRegistryClient.createFor(project, connectionData, testConnection).also {
    ...
    override fun checkConnectionInner() {
        val urlsString = kafkaProps.getProperty(CommonClientConfigs.BOOTSTRAP_SERVERS_CONFIG)
            ?: throw BdtUnexpectedConnectionException(
                null, KafkaMessagesBundle.message(
                    "connection.is.not.found.in.config",
    ...
    suspend fun listConsumerGroupOffsets(consumerGroup: String): Map<TopicPartition, OffsetAndMetadata> {
        val offsetResults =
            kafkaAdminNotNull.listConsumerGroupOffsets(consumerGroup).all().await().entries.firstOrNull()?.value
                ?: emptyMap()
        return offsetResults
    }

    fun getConsumerGroupOffsets(consumerGroup: String) =
        kafkaAdminNotNull.listConsumerGroupOffsets(consumerGroup).partitionsToOffsetAndMetadata().get()?.toMap()
            ?: mapOf()
    ...
    suspend fun getDetailedTopicsInfo(topicNames: List<String>): List<TopicPresentable> {
        ...
        val parsedInternal = describedTopics.map { description ->
            BdtKafkaMapper.topicDescriptionToInternalTopic(
                description,
                partitions = partitionsInfos[description.name()] ?: emptyList()
            )
        }
    ...
    suspend fun loadLatestOffsets(topicPartitions: Set<TopicPartition>): Map<TopicPartition, Long> {
        ...
        val request =
            kafkaAdminNotNull.listOffsets(topicPartitions.associateWith { OffsetSpec.latest() }.toMutableMap())
    ...
    private suspend fun requestTopicOffsets(topics: List<TopicDescription>): Map<String, List<BdtTopicPartition>> {
        ...
        val earliestOffsetsFuture = kafkaAdminNotNull.listOffsets(
            requestPartitions.associateWith { OffsetSpec.earliest() }.toMutableMap()
        ).all()
        val latestOffsetsFuture =
            kafkaAdminNotNull.listOffsets(requestPartitions.associateWith { OffsetSpec.latest() }.toMutableMap()).all()
    ...
    fun createTopic(name: String, numPartition: Int?, replicationFactor: Int?) {
        ...
        val newTopic =
            NewTopic(name, Optional.ofNullable(numPartition), Optional.ofNullable(replicationFactor?.toShort()))
    ...
    private fun getKafkaProps(connectionData: KafkaConnectionData): Properties {
        val defaultProps = listOf(
            ...
            Property(
                ...
                maxOf(BdIdeRegistryUtil.RFS_DEFAULT_TIMEOUT, 30_000).toString()
            ),
            Property(CommonClientConfigs.RETRIES_CONFIG, 3.toString()),
            Property(
                CommonClientConfigs.DEFAULT_API_TIMEOUT_MS_CONFIG,
                maxOf(3 * BdIdeRegistryUtil.RFS_DEFAULT_TIMEOUT, 30_000).toString()
            )
        )
        ...
        val properties =
            if (connectionData.propertySource == KafkaPropertySource.FILE && connectionData.brokerConfigurationSource == KafkaConfigurationSource.FROM_PROPERTIES) {
        ...
        if (saslCallback is String) {
            try {
                props[SaslConfigs.SASL_CLIENT_CALLBACK_HANDLER_CLASS] =
                    this::class.java.classLoader.loadClass(saslCallback)
            } catch (_: Throwable) {
                //Ignore
            }
        }
    ...
    ): MutableMap<TopicPartition, ListOffsetsResult.ListOffsetsResultInfo> {
